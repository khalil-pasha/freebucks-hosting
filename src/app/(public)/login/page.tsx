...
function LoginContent() {
  ...
  const handleLoginClick = () => {
    ...
    // Append forcePrompt=true and a nonce to backend auth URL so it prompts for account freshly
    const separator = authUrl.includes('?') ? '&' : '?';
    window.location.href = `${authUrl}${separator}forcePrompt=true&nonce=${Date.now()}`;
  };
      ...
      >
        <Card className="border-border/50 bg-card/80 backdrop-blur-md shadow-2xl relative overflow-hidden">
          ...
          <CardContent className="pt-6">
            {isManualLogout && (
              <div className="mb-6 p-4 bg-yellow-500/10 border border-yellow-500/50 rounded-sm">
                <p className="text-sm text-yellow-200/90 leading-relaxed text-center font-medium">
                  You have been logged out from FreeBucks. If Discord logs you in automatically, it means you are still signed in to Discord in this browser. To use another account, log out from Discord or use incognito.
                </p>
              </div>
            )}
