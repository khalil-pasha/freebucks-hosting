...
import { Search, MoreVertical, Ban, Coins, Eye, RefreshCw } from "lucide-react"
...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await api.get('/admin/core/users')
        setUsers(res.data)
      } catch (err) {
        console.error(err)
      } finally {
              ...
              {loading ? (
                ...
                <tr key={user.id} className="hover:bg-foreground/5 transition-colors">
                  ...
                  <td className="px-6 py-4 font-bold text-success flex items-center gap-1">
                    <Coins className="w-3 h-3" /> {user.balance?.toFixed(2)}
                  </td>
                  <td className="px-6 py-4">{user._count?.servers || 0}</td>
                  <td className="px-6 py-4 text-foreground/70">{new Date(user.createdAt).toLocaleDateString()}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${
                      user.role === "ADMIN" ? "bg-purple-500/20 text-purple-500" : "bg-success/20 text-success"
                    }`}>
                      {user.role}
                    </span>
                  </td>
