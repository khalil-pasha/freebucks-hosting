...
export class PterodactylService {
  ...
  private static getEggConfig(eggType: string) {
    const type = eggType.toLowerCase();
    
    switch (type) {
      case 'paper':
        if (!process.env.PAPER_EGG_ID && !process.env.PTERODACTYL_EGG_ID) throw new Error('Egg configuration missing for PAPER_EGG_ID');
        return {
      ...
      case 'forge':
        if (!process.env.FORGE_EGG_ID) throw new Error('Egg configuration missing for FORGE_EGG_ID');
        return {
      ...
      case 'vanilla':
        if (!process.env.VANILLA_EGG_ID) throw new Error('Egg configuration missing for VANILLA_EGG_ID');
        return {
      ...
      case 'bungeecord':
        if (!process.env.BUNGEECORD_EGG_ID) throw new Error('Egg configuration missing for BUNGEECORD_EGG_ID');
        return {
      ...
      case 'sponge':
        if (!process.env.SPONGE_EGG_ID) throw new Error('Egg configuration missing for SPONGE_EGG_ID');
        return {
  ...
  public static async createServer(name: string, ramGB: number, cpu: number, disk: number, pteroUserId: number, eggType: string = 'paper') {
    if (!this.isAppConfigured()) {
      throw new Error('Pterodactyl Application API is not configured');
    }
