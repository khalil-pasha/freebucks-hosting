...
export class ProfileController {
  ...
  public static async updateEmail(req: Request, res: Response) {
    try {
      ...
      const existing = await db.user.findFirst({ where: { email, id: { not: userId } } });
      if (existing) {
  ...
  public static async getSessions(req: Request, res: Response) {
    try {
      ...
      const dbSessions = await db.userSession.findMany({
  ...
  public static async revokeAllSessions(req: Request, res: Response) {
    try {
      const userId = req.user!.id;
      const currentSessionId = req.user!.sessionId;
  ...
  public static async updatePreferences(req: Request, res: Response) {
    try {
      const userId = req.user!.id;
      const {
        emailAlertServerExpiry,
        emailAlertServerSuspension,
        emailAlertPayment,
        emailAlertPromotional
      } = req.body;
